...
SDBU (Serial Data Base Uni) 是一种用于串口协议信号定义的二进制文件格式。
...
| 偏移 | 长度 | 字段 | 说明 |
|------|------|------|------|
| 0 | 4 | Magic | 文件标识，"sbdu" (0x73626475) |
| 4 | 2 | EnableFlags | 字段启用标志，16位，每bit对应一种字段类型 |
| 6 | 16 | ProtocolConfig | 协议配置，每字节表示一个位置，格式: [类型4bit \| 长度4bit] |
| 22 | 8 | Reserved | 预留 |
| 30 | 2 | CRC16 | 文件CRC16校验 |
| 32 | 2 | SignalCount | 信号数量 |
...
| 1 | 1 | MessageDlc | 数据长度 (字节数) |
| 2 | 32 | Name | 信号名 (UTF-8，32字节) |
| 34 | 2 | Packed | 打包数据，高9bit=长度，低9bit=起始位，bit15=字节序(0=Motorola, 1=Intel) |
| 36 | 4 | Factor | 精度 (浮点数) |
| 40 | 4 | Offset | 偏移量 (浮点数) |
| 44 | 16 | Unit | 单位 (UTF-8，16字节) |
| 60 | 1 | Flags | 标志位: b0-b1=值类型, b2=扩展帧 |
| 61 | 1 | Reserved | 预留 |
| 62 | 2 | Reserved | 预留 |
